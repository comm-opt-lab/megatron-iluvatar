...
class DistributedDataParallelConfig:
    ...
    dgc_enabled: bool = False
    """Enable top-k sparse data-parallel gradient synchronization."""

    dgc_density: float = 1.0
    """Fraction of gradient elements retained per compressed bucket."""

    dgc_momentum: float = 0.9
    """Momentum correction factor used by DGC."""

    dgc_min_numel_to_compress: int = 16384
    """Buckets smaller than this use dense all-reduce."""

    dgc_impl: str = "torch"
    """Local DGC compression implementation: torch or corex."""
    ...
    def __post_init__(self):
        ...
        if self.dgc_enabled:
            assert 0.0 < self.dgc_density <= 1.0, "dgc_density must be in (0, 1]"
            assert 0.0 <= self.dgc_momentum < 1.0, "dgc_momentum must be in [0, 1)"
            assert self.dgc_min_numel_to_compress >= 1, (
                "dgc_min_numel_to_compress must be >= 1"
            )
            assert self.dgc_impl in ("torch", "corex"), (
                "dgc_impl must be one of: torch, corex"
            )
            assert not self.use_distributed_optimizer, (
                "DGC only supports the plain all-reduce data-parallel path."
            )
            assert self.num_distributed_optimizer_instances == 1, (
                "DGC does not support multiple distributed optimizer instances."
            )
            assert not self.overlap_grad_reduce, (
                "DGC currently requires overlap_grad_reduce=False."
            )
            assert not self.average_in_collective, (
                "DGC requires average_in_collective=False."
            )
