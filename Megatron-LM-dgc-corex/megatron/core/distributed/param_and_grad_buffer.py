...
from .dgc import DGCCompressor, DGCConfig
...
class _ParamAndGradBucketGroup:
    ...
    def __init__(
        ...
        if self.ddp_config.dgc_enabled:
            self.dgc_compressor = DGCCompressor(
                DGCConfig(
                    density=self.ddp_config.dgc_density,
                    momentum=self.ddp_config.dgc_momentum,
                    min_numel_to_compress=self.ddp_config.dgc_min_numel_to_compress,
                    implementation=self.ddp_config.dgc_impl,
                )
            )
            dgc_extra_bytes = 0
            for bucket in self.buckets:
                numel = bucket.grad_data.numel()
                if self.dgc_compressor.should_compress(numel):
                    self.dgc_compressor.preallocate(bucket.grad_data)
                    dgc_extra_bytes += self.dgc_compressor.state_bytes_per_rank(
                        numel, bucket.grad_data.dtype
                    )
            if dgc_extra_bytes > 0:
                state_kind = (
                    "residual + velocity buffers"
                    if self.ddp_config.dgc_momentum > 0.0
                    else "residual buffer only"
                )
                log_on_each_pipeline_stage(
                    logger,
                    logging.INFO,
                    f"DGC enabled: {state_kind} use an extra "
                    f"{dgc_extra_bytes / (1024 ** 3):.2f} GiB per rank for this bucket group.",
                )
    ...
    def start_grad_sync(self):
        ...
        if self.dgc_compressor is not None:
            self._start_grad_sync_dgc()
            return
    ...
    @contextmanager
    def _dgc_timed(self, name: str):
        model_config = getattr(self.ddp_config, 'dgc_model_config', None)
        timers = getattr(model_config, 'timers', None) if model_config is not None else None
        if timers is None:
            yield
            return
        torch.cuda.synchronize()
        timer = timers(name, log_level=1)
        timer.start(barrier=False)
        try:
            yield
        finally:
            torch.cuda.synchronize()
    ...
    def _start_grad_sync_dgc(self):
        ...
        for bucket in self.buckets:
            ...
            numel = bucket.grad_data.numel()
            if not self.dgc_compressor.should_compress(numel):
                torch.distributed.all_reduce(
                    bucket.grad_data, op=reduce_op, group=self.data_parallel_group
                )
                continue

            with self._dgc_timed('dgc-compress'):
                indices, values = self.dgc_compressor.compress(bucket.grad_data)

            world_size = self.data_parallel_group.size()
            ...
            with self._dgc_timed('dgc-comm'):
                torch.distributed.all_gather(
                    gathered_indices, indices, group=self.data_parallel_group
                )
                torch.distributed.all_gather(
                    gathered_values, values, group=self.data_parallel_group
                )

            with self._dgc_timed('dgc-reconstruct'):
                self.dgc_compressor.reconstruct(
                    gathered_indices, gathered_values, bucket.grad_data
                )
